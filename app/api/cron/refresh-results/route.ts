...
export async function POST(request: Request) {
  ...
  // ── 5. Upsert result fields for GROUP fixtures (idempotent: onConflict "id") ──
  const BATCH = 50
  ...
  try {
    ...
    if (playedKnockout.length > 0) {
      ...
      for (const f of playedKnockout) {
        ...
        if (error) throw new Error(`Supabase knockout slot update error: ${error.message}`)
        knockoutUpdated++
      }
    }
  ...
  try {
    ...
    console.error("[cron/refresh-results] Group standings/bracket recalculation failed:", msg)
  }
